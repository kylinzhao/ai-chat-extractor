...
- [x] 6.1 前端项目：初始化前端项目（HTML + 模板引擎 或 React + Next.js）
- [x] 6.2 前端项目：配置构建工具和开发服务器
- [x] 6.3 前端项目：配置 CSS 框架（如 Tailwind CSS 或 Bootstrap）
- [x] 6.4 展示页：实现首页布局（瀑布流或列表视图）
- [x] 6.5 展示页：实现记录卡片组件（分享图、摘要、元数据）
- [ ] 6.6 展示页：实现分页或无限滚动加载
- [x] 6.7 详情页：实现单条记录详情页（`/public/:id`）
- [ ] 6.8 详情页：实现图片放大查看功能
- [x] 6.9 详情页：实现原始对话和详细汇总的折叠展示
- [x] 6.10 搜索和筛选：实现关键词搜索功能
- [x] 6.11 搜索和筛选：实现平台和日期范围筛选
- [x] 6.12 响应式设计：实现移动端、平板、桌面端适配
- [ ] 6.13 分享功能：实现图片下载和社交分享

## 7. Web 平台开发（管理后台）

- [ ] 7.1 后台认证：实现固定密钥 URL 验证逻辑
- [ ] 7.2 后台认证：实现会话 Cookie 管理和过期机制
- [x] 7.3 后台布局：实现管理后台界面框架（导航、侧边栏）
- [x] 7.4 记录列表：实现所有记录的列表展示（包括隐藏记录）
- [x] 7.5 状态管理：实现可见性切换按钮（公开/隐藏）
- [ ] 7.6 状态管理：实现批量更新可见性功能
- [ ] 7.7 内容编辑：实现社媒摘要编辑模态框
- [ ] 7.8 内容编辑：实现详细汇总编辑功能
- [ ] 7.9 内容编辑：实现重新渲染分享图功能
- [x] 7.10 记录删除：实现单条记录删除功能（带确认对话框）
- [ ] 7.11 记录删除：实现批量删除功能
...
- [x] 8.3 集成测试：编写端到端测试（插件 → 后端 → AI → 存储）
- [x] 8.4 插件测试：在 Chrome 中手动测试插件功能
- [x] 8.5 性能测试：测试 Puppeteer 渲染性能和并发能力
- [x] 8.6 安全测试：测试 API 安全（CORS、注入攻击、频率限制）
- [ ] 8.7 兼容性测试：测试不同浏览器的兼容性（Chrome、Edge）
- [x] 8.8 移动端测试：测试移动端响应式布局
