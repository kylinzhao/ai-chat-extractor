...
- [ ] 5.1 渲染服务：创建 Puppeteer 模块，封装实例管理逻辑
- [ ] 5.2 渲染服务：实现 Puppeteer 实例池（预启动 3-5 个实例）
- [ ] 5.3 渲染服务：实现任务队列（限制并发渲染数量）
- [ ] 5.4 渲染服务：实现实例获取和释放逻辑
- [ ] 5.5 HTML 模板：创建 Bento UI 模板（HTML + CSS）
- [ ] 5.6 HTML 模板：创建 Newsletter 模板（HTML + CSS）
- [ ] 5.7 HTML 模板：创建复古信纸模板（HTML + CSS）
- [ ] 5.8 模板引擎：实现变量替换逻辑（填充社媒摘要、元数据）
- [ ] 5.9 渲染逻辑：实现 HTML 加载和截图生成（@2x 高清）
- [ ] 5.10 渲染逻辑：实现图片保存到文件系统
- [ ] 5.11 渲染逻辑：实现图片 URL 生成和存储
- [ ] 5.12 渲染服务：实现错误处理和超时机制（30 秒超时）
- [ ] 5.13 渲染服务：实现重试逻辑（最多 3 次）
- [ ] 5.14 性能优化：实现模板资源缓存（CSS、字体）
- [ ] 5.15 性能监控：实现渲染性能指标记录（耗时、成功率）
