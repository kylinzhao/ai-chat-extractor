...
- [ ] 4.1 AI 客户端：创建 DeepSeek API 客户端模块（封装 HTTP 调用）
- [ ] 4.2 AI 客户端：实现 API 密钥配置和请求头设置
- [ ] 4.3 AI 客户端：实现错误处理和重试逻辑（最多 3 次）
- [ ] 4.4 提示词管理：创建提示词配置模块（支持变量替换）
- [ ] 4.5 提示词管理：实现"详细汇总"提示词模板
- [ ] 4.6 提示词管理：实现"社媒摘要"提示词模板
- [ ] 4.7 提示词管理：实现"去 AI 化"指令和禁用词检测
- [ ] 4.8 AI 调用：实现异步任务队列（处理 AI 生成请求）
- [ ] 4.9 AI 调用：实现详细汇总生成逻辑
- [ ] 4.10 AI 调用：实现社媒摘要生成逻辑
- [ ] 4.11 AI 调用：实现 Token 计数和成本计算
- [ ] 4.12 AI 监控：实现 API 调用日志记录（Token 消耗、成本、响应时间）
- [ ] 4.13 AI 缓存：实现请求缓存（相同对话不重复生成）
- [ ] 4.14 AI 限流：实现每日调用上限控制
